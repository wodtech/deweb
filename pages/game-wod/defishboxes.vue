<template>
  <div>
    ...
    >
      <span class="mb-3 text-h3 text-uppercase" style="font-family: Druk !important;">DEFISH BOX IS HERE</span>
      <p class="body-1 text-center mt-4" style="max-width: 600px;">
        Open the Fish Boxes to get equipment for fishing. Each box contains <span class="font-weight-bold">4 items</span> of equipment of
        <span class="font-weight-bold">random quality</span>.
      </p>
      <v-row class="d-flex align-center mt-4" style="width: 100%; z-index: 1;">

        <v-col md="4" class="d-flex align-center justify-center" v-for="(card, i) in lootboxes" :key="'A' + i">
          <v-card
          ...
          >
            ...
            <v-card-title class="d-flex justify-center pa-0"
                          @click="openLogin(card, i)"
            ><span class="main-title text-h3 pa-0" >{{card.name}}</span></v-card-title
            >
            <v-card-text class="mt-2">
              <v-row class="d-flex flex-column">
                <div class="d-flex flex-wrap rarity-chips py-3">
                  <div
                          v-for="(chance, name) in card.chances"
                          :key="name"
                  >
                    <v-tooltip top rounded="xl" color="#253f48">
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn height="35" v-on="on" v-bind="attrs" x-small rounded color="transparent" style="z-index: 100">
                          <div class="d-flex align-center justify-center" >
                            <RarityIcon :color="rarityColor(name)" class="mr-1" />
                            <span :style="{color: rarityColor(name) }" style="transform: translateY(2px)">{{chance}}%</span>
                          </div>
                        </v-btn>
                      </template>
                      <div class="tooltip-content d-flex align-center">
                        <CrystalIcon :rarity="rarities[name].name" :size="2" class="mr-1" style="margin-top: -3px;" />
                        <span style="text-transform: uppercase; font-weight: 600;" :style="{color: rarityColor(name) }">{{name}}</span>
                      </div>
                ...
                <v-col class="d-flex pa-0 pt-2">
                  <div style="color: #67E8D3;">{{card.limit - card.count}} left</div>
                  <div style="margin-left: auto;">{{card.limit}}</div>
                </v-col>
                <v-col class="d-flex pa-0 mt-2">
                  <v-row class="d-flex align-center lower-row">
                    <v-col cols="12" class="d-flex price-row" v-if="card.token_name === 'wod' ">
                      <div class="text-h5 font-weight-bold d-flex" style="color: white !important" >
                      ...
                      <div style="color: #A0A2C1 !important; margin-left: 3px" v-if="rate">${{(card.price * Number(rate)).toLocaleString('en-US', {minimumFractionDigits: 2})}}</div>
                    </v-col>
                    ...
                    <v-col cols="12" class="d-flex align-end justify-end btn-row">
                      <v-btn
    ...
    <LootboxPopup :showed="lootboxPopup" :single_card="single_card" :card_items="lootbox_items"/>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      async getLootboxes() {
        const lootboxes = await Axios.get(process.env['SERVICE_API_URL'] + '/lootboxes');
